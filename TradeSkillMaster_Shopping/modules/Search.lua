...
local function ScanCallback(event, ...)
	...
	elseif event == "process" then
		...
		-- Update AuctionDB from all unfiltered records before maxPrice display filter runs.
		-- This keeps price data accurate regardless of the user's search price cap.
		if TSMAPI.AuctionDB and TSMAPI.AuctionDB.UpdateFromSearchResults then
			...
			if itemID then
				...
				for _, record in ipairs(auctionItem.records) do
					...
					if itemBuyout then
						tinsert(compactRecords, {record.count, itemBuyout})
